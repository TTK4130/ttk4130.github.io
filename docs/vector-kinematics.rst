...
:math:`\vec{v}_{b/i}^i = (3, 1, 0)^\top` m/s expressed in the inertial frame. The ship rotates at a
constant yaw rate :math:`\dot\psi = 0.2` rad/s about the vertical axis, so the angular velocity is
:math:`\vec{\omega}_{b/i}^i = (0, 0, 0.2)^\top` rad/s.

A sensor is mounted on the bow, fixed to the ship at position :math:`\vec{r}_{P/b}^b = (5, 0,
0)^\top` m (5 m ahead of the ship's origin along the body x-axis).

At the instant of interest the body frame coincides with the inertial frame (:math:`R^0_b = I`), so
:math:`\vec{r}_{P/b}^i = (5, 0, 0)^\top` m.
...
.. admonition:: Solution
    ...
    The rotation adds 1 m/s in the :math:`y`-direction because the bow sweeps sideways as the ship
    yaws.
...
:math:`\psi = 90°` followed by a pure pitch rotation of :math:`\theta = 30°`. The combined rotation
matrix is
...
.. admonition:: Solution
    ...
    **Pitch** (:math:`\theta = 30°`, so :math:`\cos 30° = \tfrac{\sqrt{3}}{2}`, :math:`\sin 30° =
    \tfrac{1}{2}`):
    ...
    **Transform the vector:**
    ...
    Note that the rotation changes the direction of the velocity vector but not its magnitude —
    :math:`\lVert\mathbf{v}\rVert = \sqrt{5}` m/s in both frames.
